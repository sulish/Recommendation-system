...
datum=pd.read_csv('train1.csv')
X = datum.as_matrix(columns=['lat','lng'])
print X

# def make_var_density_blobs(n_samples=dataclus, centers=[[0,0]], cluster_std=[0.5], random_state=0):
...
# X, labels_true = make_var_density_blobs(n_samples=dataclus, centers=centers, cluster_std=densities, random_state=0)
